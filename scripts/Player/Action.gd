...
func _input(_event) -> void:
	if interfaceFinTour.sourisOnInterface : 
		...
		if Input.is_action_just_pressed("action") and Global._units.has(pointeurSouris.positionSouris) == false and interfaceFinTour.actionsRest > 0:
			...
			nvlUnite.placement("Bleu", pointeurSouris.getMiddleMouseCell(), pointeurSouris.positionSouris)	#La fonction placement a comme paramètres : couleurEquipe, position(Vector2), case(Vector2i)
