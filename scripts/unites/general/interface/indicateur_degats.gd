...
const tailles : Array = [75, 81, 85, 91, 97, 100, 105]
...
func newPopUp(valeur : int) -> void:
	position.y -= 330#Set the base position on top of the unit
	var taille : int = 0
	...
	tween.tween_property(self, "position", Vector2(position.x + 0,  position.y - 75), 1.2)
	tween.parallel().tween_property(self, "scale", Vector2(1.15, 1.15), 1.1)		#parallel permet de faire cette animation en même temps que la précédente
