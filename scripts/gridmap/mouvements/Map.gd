...
func _dijkstra(cell: Vector2i, max_distance: int, attackable_check: bool) -> Array:
		...
		for direction in  DIRECTIONS:
			...
			if grid.is_within_bounds(coordinates):
				if visited[coordinates.y][coordinates.x]:
					...
					if is_occupied(coordinates):
						if curr_unit.couleurEquipe != _units[coordinates].couleurEquipe: #Remove this line if you want to make every unit impassable 
...
func _move_active_unit(new_cell: Vector2) -> void:
	...
	_units[new_cell] = pointeurSelec.Selection
	_deselect_active_unit()
	#print(_unit_path.current_path)
...
func _select_unit(cell: Vector2i) -> void:
	...
	visuActions.draw_walkable_cells(_walkable_cells)
...
func pointeurHasMove(new_cell: Vector2i) -> void:
	...
	if pointeurSelec.Selection and pointeurSelec.Selection.is_selected:
		print(pointeurSelec.Selection.case)
		...
		visuActions.clear() # This is what clears all the colored tiles on the grid
